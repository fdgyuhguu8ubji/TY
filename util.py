...
async def take_ss(video_file, contentID=None):
    des_dir = 'Thumbs'
    ...
    thumb_name = f"{time.time()}.jpg"
    thumb_path = os.path.join(des_dir, thumb_name)

    # ✅ Use jvripper's single() to get thumbnail URL
    thumbnail_url = None
    if contentID:
        hs = HotStar()
        _, _, _, thumbnail_url = hs.single(contentID)

    # ✅ Try downloading thumbnail via proxy
    if thumbnail_url:
        try:
            connector = aiohttp.TCPConnector(ssl=False)
            async with aiohttp.ClientSession(connector=connector) as session:
                async with session.get(thumbnail_url, proxy=PROXY) as resp:
                    if resp.status == 200:
            ...
            print(f"Failed to download Hotstar thumbnail: {e}")

    # 🔁 Fallback to ffmpeg
    await downloadaudiocli([
        "./ott/ffmpeg", "-hide_banner", "-loglevel", "error",

...
PLANS_TEXT = '''**📋 Here You Will Find All of Our Premium Plans (All Available OTTs):**  
...
🔹 **Plan 4: Premium**  
  **Price:** `₹699`  
  **Validity:** `56 Days + 4 Days Extra`  
  **DRM Video Limit:** `Unlimited`

---

⚠️ **Terms and Conditions** ⚠️  
• Payments are **non-refundable**, and no refunds will be issued.  
• If the service stops working, **no compensation** will be provided.  
...
async def video_handler(bot: Client, query: CallbackQuery):
    ...
    if key in USER_DATA[query.from_user.id]:
        ...
        if drm_client:
            ...
            # ✅ Fixed: Changed 'downloaderz' to 'downloader'
            ...
            try:
                await query.message.edit(
                    f"⬆️ Please wait starting **{upload_mode}** upload \n\n**📂 Filename:**\n `{jvname}`"
                )
                sts = query.message
            except:
...
async def tg_uploader(input_str, user_id, sts_msg):
    # block drm video
    if input_str.endswith("ty_rv.zm4"):
        try:
            os.remove(input_str)
        except:
            pass
    ...
    if check_is_streamable(file_name):
        ...
        if thumb is None:
            thumb_info = await take_ss(input_str)
            if thumb_info:
                thumb = thumb_info["thumb"]
                if not duration:
                    duration = thumb_info["duration"]
        ...
        sent_msg = await client.send_video(
            ...
            progress_args=("Uploading", sts_msg, current_time, file_name)
        )
